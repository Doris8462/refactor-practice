...
public class Receipt {
    ...
    public double getTotalCost() {
        ...
        double preRateChangeAcMultiple=taxi.isAirConditioned()?PRE_RATE_CHANGE_AC_RATE:PRE_RATE_CHANGE_NON_AC_RATE;
        double postRateChangeAcMultiple=taxi.isAirConditioned()?POST_RATE_CHANGE_AC_RATE:POST_RATE_CHANGE_NON_AC_RATE;

        totalCost += Math.min(RATE_CHANGE_DISTANCE, totalKms) * preRateChangeAcMultiple * peakTimeMultiple;
        totalCost += Math.max(0, totalKms - RATE_CHANGE_DISTANCE) * postRateChangeAcMultiple * peakTimeMultiple;
